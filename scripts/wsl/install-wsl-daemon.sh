...
# ExecStart is a command line, so it uses double-quoted escaping and literal
# percent signs. It otherwise splits checkout paths containing spaces into
# separate arguments.
UNIT_EXEC_DIR="${PROJECT_DIR//\\/\\\\}"
UNIT_EXEC_DIR="${UNIT_EXEC_DIR//\"/\\\"}"
UNIT_EXEC_DIR="${UNIT_EXEC_DIR//%/%%}"

# WorkingDirectory= and EnvironmentFile= take a literal path up to end of line
# and reject surrounding quotes, so only percent signs need escaping there.
